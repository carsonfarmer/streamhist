...
def create_package_file(root, master_package, subroot, py_files, opts, subs):
    """Build the text of the file and write the file."""
    text = format_heading(1, '%s' % makename(master_package, subroot).title())
    ...
    if subs:
        ...
        text += '.. toctree::\n\n'
        for sub in subs:
            text += '    %s.%s\n' % (makename(None, subroot).title(), sub)
        text += '\n'
    ...
    if submods:
        ...
        if opts.separatemodules:
            ...
            for submod in submods:
                modfile = makename(None, makename(None, submod)).title()
                text += '   %s\n' % modfile

                # generate separate file for this module
                if not opts.noheadings:
                    filetext = format_heading(1, modfile)
                else:
                    filetext = ''
            ...
            for submod in submods:
                modfile = makename(None, makename(subroot, submod)).title().replace("_", " ")
                if not opts.noheadings:
                    text += format_heading(2, modfile)
